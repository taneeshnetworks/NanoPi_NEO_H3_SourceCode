...
disp_output_type disp_output_type_t = DISP_OUTPUT_TYPE_NONE;
...
/*
void dumpFbInfo(disp_fb_info info){
	db_msg("dumpFbInfo: addr=(%ld, %ld, %ld, %ld), size=(%d, %d), format=%d\n",
		info.addr[0], info.addr[1], info.addr[2], info.addr[3],
		info.size.width, info.size.height,
		info.format);
}

int scaleScreen(int radio, unsigned int width, unsigned int height, disp_window *dstScreen){
	dstScreen->width = (unsigned int)(width * radio * 0.01);
	dstScreen->height = (unsigned int)(height * radio * 0.01);
	dstScreen->x = (unsigned int)(width - dstScreen->width) >> 1;
	dstScreen->y = (unsigned int)(height - dstScreen->height) >> 1;
}
*/
int df_reflash_scn_win(void)
{
	//reflash the display, in A80, make the disp0 and disp1 show the same data.
	disp_output_type output_type;
	disp_layer_info layer_para;
	disp_layer_info disp_layer_para;
	unsigned int layer_id;
	unsigned long int hdmi_scale_factor = 100;
	unsigned long int cvbs_scale_factor = 100;
	int DISP_NUM = 2;
	bool disp0_used = false, disp1_used = false;
	int i = 0;
	int hdmi_pluged = 0;
	int NONE = -1;
	int switch_to = NONE;
	static char b_init = 1;

	/* check whether the hdmi is plug-in */
	hdmi_pluged = isHdmiPluged();
	output_type = getCurrentOutputType();

	/* check whether to update output mode */
	if(0 == b_init){
		if(hdmi_pluged == 1 && output_type != DISP_OUTPUT_TYPE_HDMI){
			switch_to = HDMI;
		}
		else if(hdmi_pluged != 1 && output_type != DISP_OUTPUT_TYPE_TV){
			switch_to = CVBS;
		}
	}
	else{
	...
	db_msg("hdmi pluged state %d, switch to %d\n", hdmi_pluged, switch_to);
	switch(switch_to){
	case CVBS:
		/* disable hdmi first */
		if(0 == script_fetch("df_view", "tv_scale_factor", &cvbs_scale_factor,1)){
			if((cvbs_scale_factor < 50) || (cvbs_scale_factor > 100)){
				db_msg("the tv_scale_factor isn't correct,set it to defult:no scale.\n");
				cvbs_scale_factor = 100;
			}else{
				//db_msg("fetch tv_scale_factor successfully,tv_scale_factor=%ld\n", cvbs_scale_factor);
			}
		}
		setCvbsMode(CVBS_MOD_PAL, cvbs_scale_factor, cvbs_scale_factor);
		break;
	case HDMI:
		/* disable cvbs first */
		if(0 == script_fetch("df_view", "hdmi_scale_factor", &hdmi_scale_factor,1)){
			if((hdmi_scale_factor < 50) || (hdmi_scale_factor > 100)){
				db_msg("the hdmi_scale_factor isn't correct,set it to defult:no scale.\n");
				hdmi_scale_factor = 100;
			}else{
				//db_msg("fetch hdmi_scale_factor successfully,hdmi_scale_factor=%ld\n", hdmi_scale_factor);
			}
		}
		setHdmiMode(HDMI_MOD_720P_60HZ, hdmi_scale_factor, hdmi_scale_factor);
		break;
	default:
		break;
	}
